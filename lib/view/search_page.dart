...
class _SearchPageState extends State<SearchPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return MultiBlocProvider(
      providers: [
        BlocProvider(
            create: (BuildContext context) => SearchBloc(searchState: SearchIntial(),driveRepository: driveRepository)),
        BlocProvider(
            create: (BuildContext context) => DriveBloc(driveState: DriveInitial(), driveRepository: driveRepository,))
      ],
      child: SearchView(),
    );
  }
      ...
      children: [
        BlocBuilder<SearchBloc, SearchState>(
          builder: (context, state) {
              return Column(
                children: [
                  if (!(context.read<SearchBloc>().state is SearchEnd))...[
                    if (context.read<SearchBloc>().state is SearchIntial) ...[
                      ...
                      TextButton(
                          onPressed: () async {
                          context.read<SearchBloc>().driveRepository.launchSerch();
                          },
